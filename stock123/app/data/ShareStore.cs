...
//	load share from storage
        /*
        void updateShareList(xVector shares)
        {
            if (mBuffer != null)
            {
                int off = 0;
                ...
                for (int i = 0; i < cnt; i++)
                {
                    ...
                    int c = di.getCurrentOffset();
                    s.setCode(mBuffer, c, 0);

                    di.skip(SHARE_CODE_LENGTH);
                    int offset = di.readInt();
                    //================================================
                    int p_off = mDataOffset + offset;
                    int candle_cnt = Utils.readInt(mBuffer, p_off);
                    p_off += 4;
                    s.mMarketID = Utils.readInt(mBuffer, p_off);
                    p_off += 4;

                    //if (candle_cnt > 0)
                    //s.mCandles.setSize(candle_cnt+16);
                    s.removeAllCandles();
                    //s.mCandles.removeAllElements();

                    p_off += (candle_cnt - 1) * CANDLE_SIZE;
                    //	walk backward
                    for (int j = candle_cnt; j > 0; j--)
                    {
                        candle.init(mBuffer, p_off);

                        s.addMoreCandle(candle.getOpen(), candle.getClose(), candle.getRef(), candle.getHighest(), candle.getLowest(), candle.getVolume(), candle.getDate());

                        p_off -= CANDLE_SIZE;
                    }

                    shares.addElement(s);
                    //s->setCursorScope(Share::SCOPE_2WEEKS);
                    //===s->resetCursor();
                }
            }
        }
        */
        void loadShare(byte[] code, xVector candles)
        {
            /*
            candles.removeAllElements();
            int[] offsets = {0};
            byte[] p = seekTo(code, offsets);
            if (p == null)
                return;

            //	p now is pointing to: {int:candle_cnt, [candles]}
            int off = offsets[0];
            ...
            for (int i = 0; i < candle_cnt; i++)
            {
                //stCandle* c = (stCandle*)p;
                c.init(p, off);
                candles.addElement( ->addElement(c);
                off += CANDLE_SIZE;
            }
             */
        }
